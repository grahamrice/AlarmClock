...
#define TOUCH_THRESH 3
#define HOLD_THRESH  12
#define HOLD_HOLD1   15
#define HOLD_HOLD2   14
...
void display_time(int H, int M, int S, bool p)
{
  /* Will display numbers between 0 and 99 */
  /* Negative values mean nothing is displayed */
  int temp[3];
  char _str_buffer[8];  //6 chars and a null char...
  temp[0] = H;
  temp[1] = M;
  temp[2] = S;
...
void DisplayDateTime (bool h, bool m, bool s, bool p)
{
  static int old_secs = 0;
// We show the current date and time
  DateTime now = rtc.now();

  if(old_secs != now.second()) old_secs = now.second();
  else p = false;
  
  display_time( (h) ? now.hour() : -1, (m) ? now.minute() : -1, (s) ? now.second() : -1, p);
...
{ 
  ...
  switch(current_display){
    case DISPLAY_OFF: //probs do nothing
                     break;
    case TIME_CURRENT:  DisplayDateTime (true, true, true, true);
                        if(sec_flash_counter == 2) UI_Leds |= (RAW_LEDS_1 | RAW_LEDS_3); 
                        UI_Leds |= (alarm_status & ALARM_SET) ? RAW_LEDS_W : 0;
                        break;
                        
    case TIME_SET_H:   if(sec_flash_counter < 3) DisplayDateTime (true, true, true, true);
                       else                      DisplayDateTime (false, true, true,true );
                       if(sec_flash_counter == 2) (RAW_LEDS_1 | RAW_LEDS_3);
                       UI_Leds |= (alarm_status & ALARM_SET) ? RAW_LEDS_W : 0;
                       UI_Leds |= RAW_LEDS_S;
                       break;
                       
    case TIME_SET_M:   if(sec_flash_counter < 3) DisplayDateTime (true, true, true, true);
                       else                      DisplayDateTime (true, false, true, true);
                       if(sec_flash_counter == 2) (RAW_LEDS_1 | RAW_LEDS_3);
                       UI_Leds |= (alarm_status & ALARM_SET) ? RAW_LEDS_W : 0;
                       UI_Leds |= RAW_LEDS_S;
                       break;
                       
    case TIME_ALARM_H: if(sec_flash_counter == 2) display_time(-1,alarmMinutes, -1, true);
                       else display_time(alarmHours,alarmMinutes, -1, true);
                       UI_Leds |= (alarm_status & ALARM_SET) ? RAW_LEDS_W : 0;
                       break;

    case TIME_ALARM_M: if(sec_flash_counter == 2) display_time(alarmHours,-1, -1, true);
                       else display_time(alarmHours,alarmMinutes, -1, true);
                       UI_Leds |= (alarm_status & ALARM_SET) ? RAW_LEDS_W : 0;
                       break;

    case TIME_SNOOZE: if(sec_flash_counter == 2) display_time(-1,-1, snooze_time, true);
...
int check_buttons()
{
  uint8_t buttons = 0, mask = 0, j;
  int result[2] = {0,0}, res = 0;
  static uint8_t press_complete = 0;
  bool holdholdflag = false;
  uint8_t holdholdtemp[4] = {0,0,0,0};

  if(press_complete > 0)
  {
    press_complete--; /* no button press immediately after a previous */
    return 0;
  }
  ...
  for(j = 0; j < 4; j++)
  {
    ...
    {
      button_time[j]++; //if button pressed, increment counter
      
      if(button_time[j] == -1) //still pressed and we've reached the hold threshold again, issue hold command.
      {
        result[0] |= (mask << 4);
    ...
    {
      if(button_time[j] > HOLD_THRESH ) result[0] |= (mask << 4); //we've been holding it, so issue that command
                                          //don't hold and touch at the same time
  ...
  /* If one hold has been released but not all at that moment, return all relevant bits instead of just those released */
  /* i.e. give the benefit of the doubt, otherwise we need to add some kind of release timer */
  /* Hold threshold is greater priority than touch threshold */
  if(result[0] != 0)
  {
    for(j = 0; j < 4; j++)
    {
      mask = 1 << j;
      if(button_time[j] > HOLD_THRESH) res |= (mask << 4);
      if(button_time[j] < 0) res |= (mask << 4);
      if(holdholdtemp[j] < 0){
        res |= (mask << 4);
        button_time[j] = holdholdtemp[j];
      }
    }
  }
  else if(result[1] != 0)
  {
    ...
    {
      mask = 1 << j;
      if(button_time[j] > TOUCH_THRESH) res |= mask;
    }
  }
...
{ 
  ...
  if( buttons != 0 )
  {
    ...
    if((alarm_status & ALARM_TRG) == ALARM_TRG)
    {
      ...
      if((threepress(buttons,0))||(threepress(buttons,4))) alarm_status &= ~ALARM_SET;
    ...
    }else
    {
      switch(menu_mode)
      {
              case MENU_OFF  :   if(threepress(buttons,0)) menu_mode = MENU_NONE;
                                 break; 
              case MENU_NONE :   if(buttons == BUTTON_MENU_H)
                                  ...
                                  }else if(buttons == BUTTON_ALARM_H)
                                  {
                                    ...
                                    menu_timeout = 7000;
                                  }else if(threepress(buttons,0)) menu_mode = MENU_OFF;
                                  break;
              case MENU_SET_HH : if(buttons == BUTTON_MENU)
                                  ...
                                  {
                                    rtc.adjust(current-TimeSpan(3600)); 
              ...
              case MENU_SET_MM : if(buttons == BUTTON_MENU)
                                  ...
                                  {
                                    rtc.adjust(current-TimeSpan(60)); 
                                  }else if(buttons == BUTTON_UP)
                                  {
                                    rtc.adjust(current+TimeSpan(60));                                     
                                  }
              ...
              case MENU_SAL_SN : if((buttons == BUTTON_MENU)||(buttons == BUTTON_ALARM))
                                  ...
                                  {
                                    snooze_time--;
                                    if(snooze_time < 1) snooze_time = 1;
                                  }else if(buttons == BUTTON_UP)
                                  {
                                    snooze_time++;
                                    if(snooze_time > 60) snooze_time = 60;
                                  }
                                  menu_timeout = 5000;
                                  break;
